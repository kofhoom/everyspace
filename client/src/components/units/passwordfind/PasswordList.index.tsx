...
export default function PasswordList() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      // 서버로 로그인 요청 보냄
      const { data } = await axios.post(`/auth/findPassword`, formData);
        ...
        <div className="mx-auto w-96 sm:w-full">
          ...
          {result ? (
            <div className="p-5 bg-white rounded-lg border flex flex-col">
              <p>
                사용자의 임시비밀번호는{" "}
                <span style={{ color: "#1677ff" }} className="font-bold">
                  {result}
                </span>
                입니다.
              </p>
              <Link href="/passwordchange" legacyBehavior key="login">
                <Button
                  type="primary"
                  style={{ background: "#1677ff", color: "#fff" }}
                  className="hover:border-gray-300 mt-5 py-2 mb-1 text-xm font-bold h-10 rounded-xl"
                >
                  비밀번호 변경
                </Button>
              </Link>
